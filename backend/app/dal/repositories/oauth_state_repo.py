"""OAuthStateRepository — DynamoDB access for OAuthState table."""
...
class OAuthStateRepository(BaseRepository):
    """Repository for the OAuthState table.

    Key schema: state (HASH)
    TTL: expires_at
    """
    ...
    def create_with_ttl(self, item: dict[str, Any], expires_at: int) -> dict[str, Any]:
        """Create an OAuth state record with a TTL expiration timestamp."""
