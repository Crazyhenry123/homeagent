...
def mock_users_table():
    """Patch get_table('Users') to return a mock DynamoDB table."""
    mock_table = MagicMock()
    with patch("app.agentcore_identity.get_table", return_value=mock_table) as m:
        yield mock_table


@pytest.fixture()
def registered_user(mock_users_table):
    """Configure mock Users table to return a registered user."""
    user_item = {
        "user_id": "usr_01ABC",
    ...
    mock_users_table.query.return_value = {"Items": [user_item]}
    return user_item
...
class TestValidateToken:
    ...
    def test_valid_token_returns_identity(
        self, middleware, mock_users_table, registered_user
    ):
    ...
    def test_expired_token_raises(self, middleware, mock_users_table):
    ...
    def test_wrong_issuer_raises(self, middleware, mock_users_table):
        token = _make_token(iss="https://evil.example.com")
        with pytest.raises(pyjwt.InvalidTokenError):
            middleware.validate_token(token)

    def test_wrong_audience_raises(self, middleware, mock_users_table):
    ...
    def test_user_not_registered_raises(self, middleware, mock_users_table):
    ...
    def test_member_role_default(self, middleware, mock_users_table):
...
class TestRequireAuth:
    ...
    def test_valid_token_sets_context(
        self, middleware, mock_users_table, registered_user
    ):
    ...
    def test_expired_token_returns_401_with_code(
        self, middleware, mock_users_table
    ):
        app = self._make_app_with_route(middleware)
    ...
    def test_invalid_token_returns_401(self, middleware, mock_users_table):
    ...
    def test_unregistered_user_returns_401(self, middleware, mock_users_table):
        mock_users_table.query.return_value = {"Items": []}
        app = self._make_app_with_route(middleware)
        token = _make_token(sub="unknown-sub")
        with app.test_client() as c:
    ...
    class TestDualAuthMode:
        ...
        def test_device_token_fallback_succeeds(self, middleware, mock_users_table):
            ...
            # get_table is called twice: once for "Devices", once for "Users"
            mock_users_table.query.return_value = {"Items": [device_item]}
            mock_users_table.get_item.return_value = {"Item": user_item}
        ...
        def test_device_token_fallback_no_user_returns_401(
            self, middleware, mock_users_table
        ):
        ...
        def test_expired_jwt_does_not_fallback(self, middleware, mock_users_table):
            ...
            # get_table should NOT have been called for Devices lookup
            mock_users_table.query.assert_not_called()
...
class TestRequireRole:
    ...
    def test_admin_role_allowed(self, middleware, mock_users_table, registered_user):
    ...
    def test_member_role_forbidden(self, middleware, mock_users_table):
...
class TestGetIdentityContext:
    ...
    def test_returns_context_from_g(self, middleware, mock_users_table, registered_user):
        from flask import Flask
...
class TestFamilyGroupResolution:
    ...
    def test_existing_family_group_resolved(self, middleware):
        ...
        mock_users.query.return_value = {"Items": [user_item]}

        mock_family = MagicMock()
        mock_family.query.return_value = {"Items": [family_item]}

        def table_router(name):
            if name == "Users":
                return mock_users
            if name == "FamilyGroups":
                return mock_family
            return MagicMock()
        ...
        with patch("app.agentcore_identity.get_table", side_effect=table_router):
            with app.test_client() as c:
                resp = c.get(
                    "/protected",
                    headers={"Authorization": f"Bearer {token}"},
    ...
    def test_auto_creates_family_group_when_none_exists(self, middleware):
        ...
        mock_family = MagicMock()
        mock_family.query.return_value = {"Items": []}  # No family group
        ...
        mock_family.put_item.assert_called_once()
        put_args = mock_family.put_item.call_args
    ...
    def test_family_id_from_user_record_takes_precedence(self, middleware):
        ...
        mock_family.query.assert_not_called()
    ...
    def test_family_lookup_failure_proceeds_with_none(self, middleware):
        ...
        mock_family = MagicMock()
        mock_family.query.side_effect = Exception("DynamoDB unavailable")
    ...
    def test_device_token_fallback_resolves_family_group(self, middleware):
        ...
        mock_devices.query.return_value = {"Items": [device_item]}

        mock_users = MagicMock()
        mock_users.get_item.return_value = {"Item": user_item}
